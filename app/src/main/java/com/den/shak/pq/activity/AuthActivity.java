...
import static com.den.shak.pq.cloud.VoiceAuth.sendVoiceAuth;
...
import com.den.shak.pq.cloud.GetUser;
import com.den.shak.pq.models.User;
import com.den.shak.pq.R;
import com.den.shak.pq.cloud.VoiceAuth;
...
public class AuthActivity extends AppCompatActivity implements VoiceAuth.VoiceAuthCallback, GetUser.GetUserCallback {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if (!preferencesUserID.isEmpty()) {
            ...
            Thread thread = new Thread(() -> runOnUiThread(() -> GetUser.getUserData(preferencesUserID, null, AuthActivity.this, AuthActivity.this)));
            thread.start();
        } else {
    ...
    public void onClickCheckCode(View view) {
        ...
        if (input_code.equals(auth_code)) {
            ...
            Thread thread = new Thread(() -> runOnUiThread(() -> GetUser.getUserData(null, phone, AuthActivity.this, AuthActivity.this)));
            thread.start();
        } else {
            TextInputLayout codeLayout = findViewById(R.id.code_layout);
